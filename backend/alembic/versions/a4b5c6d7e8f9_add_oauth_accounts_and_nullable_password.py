...
def upgrade() -> None:
    # Make ``hashed_password`` nullable so OAuth-only users can exist without a
    # password. Use batch mode so the ALTER is SQLite-safe as well as
    # Postgres-safe, per repo convention.
    ...
    # Link table mapping a provider identity to a MapLeads user. The unique
    # constraint on ``(provider, provider_account_id)`` ensures one provider
