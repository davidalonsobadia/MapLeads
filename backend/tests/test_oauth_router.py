...
The callback verifies the ``state`` (CSRF protection), so tests mint a real
state with ``utils.create_oauth_state`` and replay it.
...
class FakeOAuthClient:
    ...
    def fetch_user_info(self, access_token: str) -> OAuthUserInfo:
        # Guard the trap where a fake built without ``info`` would otherwise
        # return ``None`` and turn a callback test into an opaque 500.
        if self._info is None:
            raise OAuthError(
                "FakeOAuthClient.fetch_user_info called with info=None; "
                "pass info= or set exchange_error=True."
            )
        return self._info
...
def test_callback_unknown_provider_returns_400(db_client):
    response = db_client.post(
        f"{AUTH}/oauth/microsoft/callback",
        json={"code": "c", "state": utils.create_oauth_state("microsoft")},
    )
...
def test_callback_new_user_creates_rows(db_client, use_oauth_client, db_session):
    ...
    response = db_client.post(
        ...
        json={"code": "abc", "state": utils.create_oauth_state("google")},
    )
    assert response.status_code == 200
    body = response.json()
...
def test_callback_existing_link_returns_same_user_without_new_rows(
    ...
    response = db_client.post(
        f"{AUTH}/oauth/github/callback",
        json={"code": "c", "state": utils.create_oauth_state("github")},
    )
    assert response.status_code == 200
...
def test_callback_exchange_failure_returns_502(db_client, use_oauth_client):
    ...
    response = db_client.post(
        f"{AUTH}/oauth/google/callback",
        json={"code": "bad", "state": utils.create_oauth_state("google")},
    )
    assert response.status_code == 502


def test_callback_missing_state_returns_422(db_client, use_oauth_client):
    use_oauth_client(FakeOAuthClient(info=_info()))
    response = db_client.post(f"{AUTH}/oauth/google/callback", json={"code": "c"})
    assert response.status_code == 422


def test_callback_invalid_state_returns_400(db_client, use_oauth_client):
    use_oauth_client(FakeOAuthClient(info=_info()))
    response = db_client.post(
        f"{AUTH}/oauth/google/callback",
...
def test_callback_state_for_other_provider_returns_400(db_client, use_oauth_client):
    # A state minted for github must not be accepted on the google callback.
    use_oauth_client(FakeOAuthClient(info=_info()))
    response = db_client.post(
        f"{AUTH}/oauth/google/callback",
        json={"code": "c", "state": utils.create_oauth_state("github")},
...
def test_issued_token_authorizes_me(db_client, use_oauth_client):
    ...
    token = (
        db_client.post(
            f"{AUTH}/oauth/google/callback",
            json={"code": "c", "state": utils.create_oauth_state("google")},
        )
        .json()["access_token"]
    )
