...
def _oauth_redirect_uri(provider: str) -> str:
    ...
    ``{FRONTEND_URL}/api/auth/oauth/{provider}/callback``. ``FRONTEND_URL`` is
    ``rstrip``-ed so a configured trailing slash does not produce a double slash
    (OAuth providers match redirect URIs exactly).
    """
...
def oauth_callback(
    ...
    The ``state`` is verified first (CSRF protection): it must be a non-expired
    token this server signed for ``provider``, else 400. Provider/transport
    failures from the client surface as 502; the link-or-create rules (and the
    400 for an unverified provider email) live in the service.
    """
    utils.verify_oauth_state(data.state, provider)
    redirect_uri = _oauth_redirect_uri(provider)
    try:
