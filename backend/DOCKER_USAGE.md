...
```bash
# Backup
docker-compose exec db pg_dump -U postgres map_leads_db > backup.sql

# Restore
docker-compose exec -T db psql -U postgres -d map_leads_db < backup.sql
```
