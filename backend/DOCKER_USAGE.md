...
```bash
# Backup
docker-compose exec db pg_dump -U postgres mapleads_db > backup.sql

# Restore
docker-compose exec -T db psql -U postgres -d mapleads_db < backup.sql
```
